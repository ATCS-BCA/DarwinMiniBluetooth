...
public class AddCRC {
	private static int[] crc_table = new int[] { 0x0000, 0x8005, 0x800F, 0x000A, 0x801B, 0x001E, 0x0014, 0x8011, 0x8033,
	...
	/**
	 * 
	 * @param packet	The complete packet with the command.
	 * @return crcHL 	Returns the crcHigh crcLow
	 */
	public static String addCrc(String packet) {
		// set the current crc to be blank
		int crc = 0x0;

		// take the packet and convert it to a byte[]
		byte[] buffer = new BigInteger(packet, 16).toByteArray();

		// calculate the crc *some math*
		for (int j = 0; j < buffer.length; j++) {
			int i = ((crc >> 8) ^ buffer[j]) & 0xff;
			crc = (crc << 8) ^ crc_table[i];
		}

		// get the string representation and add 0x10000
		String crc16 = Integer.toHexString(crc + 0x10000);

		// get the last 4 bytes of the crc
		crc16 = crc16.substring(crc16.length() - 4, crc16.length());

		// set the first 2 values of the string as crcHigh and last 2 as crcLow
		String crcHigh = crc16.substring(0, 2);
		String crcLow = crc16.substring(2, 4);

		// return the crc in the low first then high
